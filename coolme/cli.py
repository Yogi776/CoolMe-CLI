...
@click.option('--output-tables', type=lambda kv: {k:v for k, v in (x.split('=') for x in kv.split(','))}, required=True,
              help="Mapping of ingestion items to output tables for Azure PostgreSQL")
...
def azure_postgres(name,project_name, ingestion_items, output_catalog, output_schema, output_tables, template_path):
    """Create ingestion files specifically configured for Azure PostgreSQL."""
    logging.info(f"Creating Azure PostgreSQL workflows for project: {project_name}")
    base_path = os.path.join(os.getcwd(), project_name, f"{name}/build/data-processing")
    create_folder_if_not_exists(base_path)
    ...
    for item in ingestion_items:
        ...
        file_path = os.path.join(base_path, f"cofig-{item}-flare.yaml")
        with open(file_path, 'w') as file:
            file.write(file_content)
            logging.info(f"Created Azure PostgreSQL workflow file: {file_path}")
